...
from app.utils.tg_bot import send_tg_message
        ...
        if old_avatar and old_avatar["avatar"]:
            ...
            if os.path.exists(old_path):
                try:
                    ...
                    print(f"⚠️ Не удалось удалить старый аватар: {old_path}")
        ...
        if user and user["avatar"]:
            ...
            if os.path.exists(avatar_path):
                try:
                    ...
                    print(f"⚠️ Не удалось удалить аватар: {avatar_path}")
...
async def forgot_password(email: str):
    ...
    if not user:
        return {
            "message": "Если email зарегистрирован, администратор получит запрос"
        }
...
тт
    ...
    await send_tg_message(
        f"🔐 <b>ЗАПРОС СБРОСА ПАРОЛЯ</b>\n"
        f"👤 Email: {email}\n"
        f"🔗 <a href='{reset_url}'>Ссылка для сброса</a>\n"
        f"⏰ Активна 24 часа"
    )

    return {"message": "Запрос на сброс пароля отправлен администратору"}
